...
export default function EditProduct({setOpen,open,GetCategoryData,id}) {
  ...
  const SubmitCategoryData = async ()=>{
    try {
      const resp = await authFetch.patch(`/api/admin/edit-category/${id}`,{title:categoryData.title});
  ...
  const GetSingleCategoryData = async ()=>{
    try {
      const resp = await authFetch(`/api/admin/get-category/${id}`);
      setCategoryData(resp.data.data)
    } catch (error) {
      console.log(error)
    }
